...
public class Bytes {
	...
	/**
	 * Returns length of the byte array, returning 0 if the array is null. Useful
	...
	/**
	 * Add the whole content of the ByteBuffer to the bytes arrays. The ByteBuffer
	 * is modified.
	 *
	 * @param bytes  the byte array
	 * @param offset position in the array
	 ...
	 * 0 (inclusive) to the limit (exclusive), regardless of the current position.
	...
	public static byte[] toBytes(DataOutputByteBuffer bos) {
		ByteBuffer buf = ByteBuffer.allocate(bos.getLength());
		for (ByteBuffer b : bos.getData()) {
			buf.put(b);
		}
		buf.flip();
		return buf.array();
	...
	public static DataInputByteBuffer toBis(byte[] data) {
	...
	public static long toLong(byte[] bytes, int offset, final int length) {
		...
		if (org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.isAvailable()) {
	...
	public static int putLongUnsafe(byte[] bytes, int offset, long val) {
		if (org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.littleEndian) {
			val = Long.reverseBytes(val);
		}
		org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.theUnsafe.putLong(bytes,
				(long) offset
						+ org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.BYTE_ARRAY_BASE_OFFSET,
				val);
		return offset + SIZEOF_LONG;
	}
	...
	public static int toIntUnsafe(byte[] bytes, int offset) {
		if (org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.littleEndian) {
			return Integer.reverseBytes(
					org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.theUnsafe
							.getInt(bytes, (long) offset
									+ org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.BYTE_ARRAY_BASE_OFFSET));
		} else {
			return org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.theUnsafe
					.getInt(bytes, (long) offset
							+ org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.BYTE_ARRAY_BASE_OFFSET);
		}
	}
	...
	public static int putIntUnsafe(byte[] bytes, int offset, int val) {
		...
		org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.theUnsafe.putInt(bytes,
				(long) offset
						+ org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.BYTE_ARRAY_BASE_OFFSET,
				val);
		return offset + SIZEOF_INT;
	 ...
	 * @return the short value
	 * @throws IllegalArgumentException if length is not {@link #SIZEOF_SHORT} or if there's not enough
	 *                                  room in the array at the offset indicated.
	 */
	...
	public static int putShort(byte[] bytes, int offset, short val) {
		if (bytes.length - offset < SIZEOF_SHORT) {
			throw new IllegalArgumentException(
					"Not enough room to put a short at" + " offset " + offset + " in a " + bytes.length + " byte array");
		}
		if (org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.isAvailable()) {
	...
	public static int putShortUnsafe(byte[] bytes, int offset, short val) {
		...
		org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.theUnsafe.putShort(bytes,
				(long) offset
						+ org.mmo.server.common.utils.Bytes.LexicographicalComparerHolder.UnsafeComparer.BYTE_ARRAY_BASE_OFFSET,
				val);
		return offset + SIZEOF_SHORT;
	...
	static class LexicographicalComparerHolder {
		...
		static Comparer<byte[]> getBestComparer() {
			try {
				...
				@SuppressWarnings("unchecked") Comparer<byte[]> comparer = (Comparer<byte[]>) theClass
						.getEnumConstants()[0];
				return comparer;
	...
	public static boolean startsWith(byte[] bytes, byte[] prefix) {
		return bytes != null && prefix != null && bytes.length >= prefix.length
				&& LexicographicalComparerHolder.BEST_COMPARER.compareTo(bytes, 0, prefix.length, prefix, 0, prefix.length)
				== 0;
	}
	 ...
	 * @return Runs
	 * {@link org.apache.hadoop.io.WritableComparator#hashBytes(byte[], int)}
	 * on the passed in array. This method is what
	 * {@link org.apache.hadoop.io.Text} and
	 * {@link org.apache.hadoop.hbase.io.ImmutableBytesWritable} use
	 ...
	 * @param a         Beginning of range
	 * @param b         End of range
	 * @param inclusive Whether the end of range is prefix-inclusive or is considered an
	 *                  exclusive boundary. Automatic splits are generally exclusive and
	 *                  manual splits with an explicit range utilize an inclusive end of
	 *                  range.
	 * @param num       Number of times to split range. Pass 1 if you want to split the
	 *                  range in two; i.e. one split.
	 * @return Array of dividing values
	 ...
	 * Otherwise, a value -(i + 1) such that the key is between arr[i - 1]
	 * and arr[i] non-inclusively, where i is in [0, i], if we define
	 * arr[-1] = -Inf and arr[N] = Inf for an N-element array. The above
	 * means that this function can return 2N + 1 different values ranging
	 * from -(N + 1) to N - 1.
	 */
	...
	public static void writeStringFixedSize(final DataOutput out, String s, int size) throws IOException {
		...
		if (b.length > size) {
			throw new IOException(
					"Trying to write " + b.length + " bytes (" + toStringBinary(b) + ") into a field of length " + size);
		}
	...
	/**
	 * Copy the byte array given in parameter and return an instance of a new byte
	 * array with the same length and the same content.
	 *
	...
	/**
	 * Search sorted array "a" for byte "key". I can't remember if I wrote this or
	 ...
	 * {@code java.util.Arrays.copyOfRange(array, i, i + target.length)} contains
	 ...
	 * @param array  an array of {@code byte} values, possibly empty
	 * @param target a primitive {@code byte} value
	 * @return {@code true} if {@code target} is present as an element anywhere in
	 * {@code array}.
	 */
	...
	/**
	 * Create a byte array from a string of hash digits. The length of the string
	...
	public static byte[] fromHex(String hex) {
		...
		for (int i = 0; i < b.length; i++) {
			b[i] = (byte) ((toBinaryFromHex((byte) hex.charAt(2 * i)) << 4) + toBinaryFromHex(
					(byte) hex.charAt((2 * i + 1))));
		}
		return b;
	}
