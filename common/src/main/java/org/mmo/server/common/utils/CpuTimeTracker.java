...
@InterfaceAudience.Private @InterfaceStability.Unstable public class CpuTimeTracker {
	public static final int UNAVAILABLE = ResourceCalculatorProcessTree.UNAVAILABLE;
	...
	public float getCpuTrackerUsagePercent() {
		...
		if (sampleTime > lastSampleTime + MINIMUM_UPDATE_INTERVAL) {
			cpuUsage = ((cumulativeCpuTime.subtract(lastCumulativeCpuTime)).floatValue()) * 100F / ((float) (sampleTime
					- lastSampleTime));
			lastSampleTime = sampleTime;
			lastCumulativeCpuTime = cumulativeCpuTime;
		}
